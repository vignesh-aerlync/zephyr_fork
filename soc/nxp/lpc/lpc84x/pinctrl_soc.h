...
#define Z_IOCON_PD(node_id) IF_ENABLED(DT_PROP(node_id, bias_pull_down), (1U))

#define Z_IOCON_PU(node_id) IF_ENABLED(DT_PROP(node_id, bias_pull_up), (2U))

#define Z_PINCTRL_IOCON_BIAS(node_id) (Z_IOCON_PD(node_id) | Z_IOCON_PU(node_id))

#define Z_PINCTRL_IOCON_PINCFG(node_id)                                                            \
	((Z_PINCTRL_IOCON_BIAS(node_id) << 0) | (DT_PROP(node_id, nxp_hysteresis) << 2) |          \
	 (DT_PROP(node_id, nxp_invert) << 3) | (DT_PROP(node_id, drive_open_drain) << 4))
