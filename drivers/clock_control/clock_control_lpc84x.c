...
#define LPC84X_SYSAHBCLKCTRL0_OFFSET 0x80U

/* The NXP SDK uses a global SYSCON pointer. */
...
static int lpc84x_clock_on(const struct device *dev, clock_control_subsys_t sub)
{
	...
	volatile uint32_t *reg = (volatile uint32_t *)((uint8_t *)config->base +
						       LPC84X_SYSAHBCLKCTRL0_OFFSET + reg_offset);
