...
#define SOC_NV_FLASH_NODE(inst) DT_INST_CHILD(inst, flash_0)
...
static int flash_lpc84x_iap_read(const struct device *dev, off_t offset, void *data_ptr, size_t len)
{
	...
	memcpy(data_ptr, (void *)((uint32_t)(data->flash_base) + (uint32_t)offset), len);
				  ...
				  size_t len)
{
	...
	dst = (uint32_t)(data->flash_base) + (uint32_t)offset;
	...
	while (remaining > 0) {
		...
		if (chunk == 0) {
			LOG_ERR("write: no valid IAP chunk for %zu bytes", remaining);
			ret = -EINVAL;
			break;
		}
...
static int flash_lpc84x_iap_init(const struct device *dev)
{
	...
	data->flash_base = DEVICE_MMIO_NAMED_GET(dev, flash_base);
	...
	if (ret < 0) {
		...
		POWER_EnablePD(kPDRUNCFG_PD_FLASH);
		return ret;
	}
