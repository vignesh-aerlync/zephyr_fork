...
#if defined(CONFIG_INPUT_LPC845_CAP_TOUCH)
void lpc84x_acomp_irq_handler(const struct device *dev)
#else
...
#if !defined(CONFIG_INPUT_LPC845_CAP_TOUCH)
#define LPC84X_ACOMP_CONNECT_IRQ(inst)                                                             \
	IRQ_CONNECT(DT_INST_IRQN(inst), DT_INST_IRQ(inst, priority), lpc84x_acomp_irq_handler,     \
