...
static void lpc84x_adc_start_channel(const struct device *dev)
{
	...
	data->adc_seqconfig.triggerMask = config->trigger_source;
	data->adc_seqconfig.triggerPolarity = config->trigger_polarity;
	...
	} else if (config->trigger_source == 0U) {
...
#define LPC84X_ADC_INIT(inst)                                                                      \
	...
	static const struct lpc84x_adc_config lpc84x_adc_config_##inst = {                         \
		...
		.trigger_source = DT_INST_PROP_OR(inst, nxp_trigger_source, 0),                    \
		.trigger_polarity = DT_INST_PROP_OR(inst, nxp_trigger_polarity, 1),                \
	};                                                                                         \
                                                                                                   \
